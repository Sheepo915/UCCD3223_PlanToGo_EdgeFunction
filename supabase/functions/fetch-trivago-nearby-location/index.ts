 ...
 *
 * @example
 * const nearbyLocations = await fetchNearbyLocations("https://api.content.tripadvisor.com/api/v1/location/nearby_search", {
...
async function fetchNearbyLocations(
  ...
  const url = `${baseUrl}/nearby_search?${queryParams.toString()}`;
...
Deno.serve(async (req) => {
  ...
  try {
    ...
    request = {
      latLong: encodeURI(request.latLong),
      category: request.category ? request.category : undefined,
      phone: request.phone ? encodeURI(request.phone) : undefined,
      address: request.address ? encodeURI(request.address) : undefined,
      radius: request.radius ? encodeURI(request.radius) : undefined,
      radiusUnit: request.radiusUnit ? request.radiusUnit : undefined,
      language: request.language ? encodeURI(request.language) : undefined,
    };
  } catch (error) {
  ...
  try {
    ...
    const nearbyLocations: SearchResponse = await fetchNearbyLocations(
      BASE_URL,
      request,
    );

    const compiledData = await Promise.all(
      nearbyLocations.data.map(async (location) => {
        const locationId = location.location_id;

        const [locationDetails, locationPhotos, locationReviews] = await Promise.all([
...
/* To invoke locally:
  ...
  curl -i --location --request POST 'http://127.0.0.1:54321/functions/v1/fetch-trivago-nearby-location' \
