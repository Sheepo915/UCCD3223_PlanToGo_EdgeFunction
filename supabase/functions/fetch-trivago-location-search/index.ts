...
async function fetchLocationSearch(
  ...
  try {
    ...
    if (!response.ok) {
      throw new Error(
        `HTTP error! Status: ${response.status} Response: ${JSON.stringify(result)}`,
      );
    }
...
Deno.serve(async (req) => {
  ...
  try {
    ...
    request = {
      searchQuery: encodeURI(request.searchQuery),
      latLong: request.latLong ? encodeURI(request.latLong) : undefined,
  ...
  try {
    ...
    const nearbyLocations: SearchResponse = await fetchLocationSearch(
      BASE_URL,
      request,
    );
...
/* To invoke locally:
  ...
  curl -i --location --request POST 'http://127.0.0.1:54321/functions/v1/fetch-trivago-location-search' \
